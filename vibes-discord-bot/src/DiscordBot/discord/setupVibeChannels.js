...
export default async function ({ client, guild_id }) {
  ...
  for (const channel_values of Object.values(VIBE_CHANNELS)) {
    if (channel_names.includes(channel_values.name) && VIBE_CHANNEL_RELOAD[channel_values.name]) {
      console.log(`Deleting old channel with name ${channel_values.name}`);
      const channel = guild.channels.cache.find((channel) => channel.name === channel_values.name);
