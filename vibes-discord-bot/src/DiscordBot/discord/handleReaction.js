...
import {
  GOOD_VIBE_EMOJI_NAMES,
  BAD_VIBE_EMOJI_NAMES,
  BOOSTER_VIBE_EMOJI_NAMES,
} from "../constants";
...
async function handleVibeReaction({
  ...
  if (
    ["BadVibe"].includes(entryType) &&
    note?.trim?.().match("^!(susvibe|susvibes|badvibes|badvibe)")
  ) {
...
export default async function handleReaction(client, reaction, user) {
  ...
  if (
    ...
    entryType = "BadVibe";
  } else if (BOOSTER_VIBE_EMOJI_NAMES.includes(reaction.emoji.name)) {
    entryType = "VibeBoost";
  } else {
    return;
  }
